...
<head>
  <title>CMU 15-445/645 筆記 Storage Models & Compression</title>
</head>
...
relational model 並沒有強調同一個 tuple 的所有屬性都要被儲存在同一個 page 中，
...
如同我們之前所學的一樣，NSM 會將一個 tuple 的屬性在 page 中連續儲存，
這種儲存方式較為適合 OLTP 的應用場景。

![N-ary Storage Model (NSM)](/img/cmu15-445/c05/image-2.png)

如圖所示，一般的 OLTP 應用場景中，我們會有大量的短小的查詢，我們可以直接透過 index 之後返還所有資料。
...
透過這種方式，我們就可以減少不必要的 I/O，只讀取我們需要的部分屬性，
但這樣也會造成新的問題，
...
大多數的 DBMS 都會使用一般的壓縮算法，如 gzip、snappy、lz4、Zstd 等，
主要考量的點是壓縮率和壓縮 / 解壓縮速度。

![Naive Compression](/img/cmu15-445/c05/image-9.png)

一個有名的壓縮例子是 MySQL 的 InnoDB，InnoDB 會將 page 壓縮成 1、2、4、8 KB 再傳送到 buffer pool 中，
...
在理想請況下，我們希望可以在讀資料的時候不需要對整個 page 解壓縮，
...
對於每個不同的值，我們可以建立一個 bitmap 來表示是否有這個值，
