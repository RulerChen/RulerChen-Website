...
<head>
  <title>CMU 15-445/645 筆記 Modern SQL</title>
</head>
...
```SQL
SELECT * FROM student AS s JOIN enrolled AS e ON s.sid = e.sid;
```
...
Aggregates function 不能與其他欄位同時出現在 SELECT 子句中，
因此下面的寫法是錯誤的
...
包含了 inner query 以及 outer query，
inner query 會先執行，然後再執行 outer query，
且 inner query 可以使用 outer query 中的欄位。
...
- [Notes 1](https://zhenghe.gitbook.io/open-courses/cmu-15-445-645-database-systems/advanced-sql)
