...
10. 安裝 fzf，提供 fuzzy search 以及歷史紀錄搜尋功能
...
11. 如果想在 VSCode 中使用 zsh 和 fzf，可以在 VSCode 的設定中加入以下的設定
...
其中目錄的權限比較特別，你能不能刪除一個檔案，看的不是你對該檔案有沒有寫入權限，而是你對該檔案所在的目錄有沒有寫入權限。

可以透過 `ls -l` 來查看檔案的權限，會顯示成 `-rwxr-xr--` 的格式，第一個字母代表檔案類型 (如 `-` 代表一般檔案，`d` 代表目錄)，接下來的三個字母代表擁有者的權限，再來的三個字母代表群組的權限，最後三個字母代表其他人的權限。

* `chmod`：改變檔案的權限，`chmod 755 filename`、`chmod u=rwx,g=rx,o=r filename`、`chmod +x filename` (加上執行權限) 或是 `chmod a-w filename` (移除所有人的寫入權限)
