...
接著在 `index.js` 中建立一些非常簡單的 api，並加上一個 middleware 來記錄 log
...
PostgreSQL 的 log 則是透過 `local.file_match` 來監聽 `/var/log/*.json` 這個目錄，接著處理好 timestamp 並加上兩個 label，最後加上 service_name 並送到 loki 儲存
...
在 docker-compose 中，我們啟用了 PostgreSQL 的 log，並且將 log 存放在 `/logs` 中，依照分鐘數來保存 log，因此只會儲存最近 60 分鐘的 log，會以 json 的格式儲存
